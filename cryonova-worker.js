...
// ── /load ───────────────────────────────────────────────────────────
// Verifies username + password + hwid + fingerprint, then returns the
// raw Lua payload. All mismatches return JSON error envelopes the
// loader can parse.
async function handleLoad(req, env) {
    ...
    if (!user) return json({ ok: false, error: 'unknown user' }, 403);
    ...
    if (!timingSafeEqual(user.hwid, hwid)) {
        return json({ ok: false, error: 'hwid mismatch (steam64)' }, 403);
    }
    if (user.fingerprint && fingerprint &&
        !timingSafeEqual(user.fingerprint, fingerprint)) {
        return json({ ok: false, error: 'hwid mismatch (machine)' }, 403);
    }

    user.last_seen = Date.now();
    await env.CRYONOVA_KV.put(userKey(username), JSON.stringify(user));

    // Payload pulled from a secret OR from KV at  payload:current
    let payload = env.SCRIPT_PAYLOAD;
    if (!payload) {
        payload = await env.CRYONOVA_KV.get('payload:current');
    }
    if (!payload) {
